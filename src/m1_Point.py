...
class Point(object):
    ...
    def move_to(self, x, y):
        ...
        self.distance = self.get_distance_from(p) + self.distance
    ...
    def get_distance_from_start(self):
        length = self.startx - self.x
        width = self.starty - self.y
    ...
    def closer_to(self, p2, p3):
        if self.get_distance_from(p2) > self.get_distance_from(p3):
            return p3
        elif self.get_distance_from(p3) > self.get_distance_from(p2):
            return p2
        elif self.get_distance_from(p2) == self.get_distance_from(p3):
            return p2

    def halfway_to(self, p2):
        p3 = Point((p2.x + self.x) / 2, (p2.y + self.y) / 2)
        return p3
